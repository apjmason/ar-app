...
public class convertToBNG : MonoBehaviour {

	public float gpsAccuracy = 5;
	public float gpsUpdateDistance = 1;
	public float falseEasting = 212500;
	public float falseNorthing = 75000;
	...
	//GUI Variables
	private string BNG_E_input = "218178";
	private string BNG_N_input = "80101.90";
	private string BNG_alt_input = "292";
	...
	public GameObject landscape = GameObject.Find("blender_dtm_10k_lores");

	public GameObject huts = GameObject.Find("huts");
	public GameObject spheres = GameObject.Find("spheres");
	...
	public void MoveCameraToGameSpace(double raw_BNG_E, double raw_BNG_N, double raw_BNG_alt, double heading = 9999.99){
		...
		public void OnGUI() {
			...
			if (GUI.Button (new Rect (10,140,300,100), "Reset Position and Heading")) { 
			...
			if (GUI.Button (new Rect (10,240,300,100), "Show/Hide Landscape")) { 
				if (landscape.GetComponent<Renderer>().material.shader == diffuse)
					landscape.GetComponent<Renderer>().material.shader = transwalls;
				else
					landscape.GetComponent<Renderer>().material.shader = diffuse;
			}
		if (GUI.Button (new Rect (10,450,300,100), "Set position via GPS")) {
		...
		if (GUI.Button (new Rect (1350,140,50,50), "Test 1")) { 
			huts.SetActiveRecursively(false);
			spheres.SetActiveRecursively(false); 
		}
		if (GUI.Button (new Rect (1350,210,50,50), "Test 2")) { 
			huts.SetActiveRecursively(false);
			spheres.SetActiveRecursively(true); 
		}
		if (GUI.Button (new Rect (1350,280,50,50), "Test 3")) { 
			huts.SetActiveRecursively(true);
			spheres.SetActiveRecursively(false); 
		}
	} 
